...
import os
import re
from BeyondChaosRandomizer.BeyondChaos.utils import dialoguetexttable, bytes_to_dialogue, utilrandom as random, \
    open_mei_fallback as open, read_multi, write_multi
...
def set_dialogue_var(key, value):
    dialogue_vars[key.lower()] = value


def set_dialogue_flag(flag, value=True):
    if value:
        dialogue_flags.add(flag.lower())
    elif flag in dialogue_flags:
        dialogue_flags.remove(flag)
...
def set_pronoun(name, gender, force=True):
    ...
    if "random" in gender:
        force = True
        opts = ["male"] * 9 + ["female"] * 9 + ["neutral"] * 2 if "truerandom" not in gender else ["male", "female",
                                                                                                   "neutral"]
        if gender == "orandom":
            opts += ["object"] * 20
...
def load_patch_file(file_name):
    filepath = os.path.join('data', 'script', file_name + ".txt")
    ...
    try:
        ...
        print(f"failed to open data/script/{file_name}.txt")
        return
    for i, line in enumerate(lines):
        ...
        try:
            ...
            print(f"{file_name}.txt: line {i} - {s[0]} is not a valid caption index")
            continue
        changes = s[1].rstrip('\n').split('|')
        for c in changes:
            if '->' not in c:
            ...
            if '@' in chgfrom:
                ...
                try:
                    ...
                    print(f"{file_name}.txt: line {i} - {match_idx} is not a valid match index")
                    match_idx = 0
            else:
                match_idx = None
            if chgto == "*":
...
def read_dialogue(infile_rom_buffer: BytesIO):
    # load existing script & pointer table
    infile_rom_buffer.seek(0xD0000)
    script_bin = infile_rom_buffer.read(0x1F0FF)

    # TODO battle script

    infile_rom_buffer.seek(0xCE600)
    bankidx = read_multi(infile_rom_buffer, 2)
    for idx in range(0xC0C):  # C0D through CFF pointers are repurposed
        script_ptrs[idx] = read_multi(infile_rom_buffer, 2) + (0x10000 if idx >= bankidx else 0)
...
def manage_dialogue_patches(outfile_rom_buffer: BytesIO):
    global script_bin
    ...
    # write to file
    outfile_rom_buffer.seek(0xD0000)
    assert len(new_script) <= 0x1F0FF
    outfile_rom_buffer.write(new_script)

    outfile_rom_buffer.seek(0xCE600)
    write_multi(outfile_rom_buffer, first_high_index)
    assert len(new_ptrs) <= 0x19FE
    outfile_rom_buffer.write(new_ptrs)
...
def read_location_names(infile_rom_buffer: BytesIO):
    # load existing script & pointer table
    infile_rom_buffer.seek(0xEF100)
    location_name_bin = infile_rom_buffer.read(0x4ff)

    infile_rom_buffer.seek(0x268400)
    for idx in range(0x49):
        location_name_ptrs[idx] = read_multi(infile_rom_buffer, 2)
...
def write_location_names(outfile_rom_buffer: BytesIO):
    ...
    # write to file
    outfile_rom_buffer.seek(0xEF100)
    assert len(new_location_names) <= 0x4FF
    outfile_rom_buffer.write(new_location_names)

    outfile_rom_buffer.seek(0x268400)
