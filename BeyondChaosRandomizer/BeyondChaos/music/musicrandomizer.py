...
CUSTOM_MUSIC_PATH = os.path.join(BASE_PATH, 'custom')
...
class Tracklist:
    ...
    def dupe_check(self, name, module="unknown"):
        if name in self.data:
            print(
                f"warning: in {module}: duplicate tracklist data for {name}, overwriting entry with file {self[name].file}")
            return True
        return False
    ...
    def add_random(self, name, pool, idx=None, allow_duplicates=False):
        ...
        if hasattr(sys, "_MEIPASS"):
            paths_to_search = [CUSTOM_MUSIC_PATH, LEGACY_MUSIC_PATH, resource_path(CUSTOM_MUSIC_PATH),
                               resource_path(LEGACY_MUSIC_PATH), resource_path(STATIC_MUSIC_PATH)]
        else:
...
def process_music(inrom, meta={}, f_chaos=False, f_battle=True, opera=None, eventmodes="",
    ...
    while not processing_complete:
            ...
            while prog_attempts < 1000:
                ...
                for i in (0, 3, 1, 2):
                    ...
                    if track not in track_pools:
                        prog_attempts = 1000
                        break
                    track_pool = [s for s in track_pools[track] if
                                  song_usage_id(s) not in temp_used_song_names and intensitytable[cat][
                                      s] >= prog_level and intensitytable[cat][s] <= prog_max]
        ...
        for tl_name, tl_entry in tracklist.data.items():
            ...
            if tl_name in APPENDTRACKS:
                append_type_map = {
                    ...
                    "assault": "wind",
                    "zozo": "rain"}
                tl_entry.mml = apply_variant(tl_entry.mml, append_type_map[tl_name], name=tl_entry.file,
                                             variant=variant, check_size=True if tl_name == "assault" else False)
        ...
        # -- run insertmfvi
        try:
            outrom = insertmfvi(inrom, virt_sample_list=sample_virtlist, virt_seq_list=mml_virtlist,
                                freespace=freespace, brrpath=resource_path(SAMPLE_PATH), quiet=True)
...
def process_map_music(data):
    ...
    def adjust_event(dat, offset, oldid, newid):
        op_lengths = {}
        for o in [0x38, 0x39, 0x3A, 0x3B, 0x45, 0x47, 0x49, 0x4A, 0x4E, 0x4F, 0x54, 0x5B, 0x5C, 0x7B, 0x82, 0x8E, 0x8F,
                  0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x9A, 0x9D, 0xA2, 0xA6, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,
                  0xAD, 0xAE, 0xAF, 0xB1, 0xBB, 0xBF, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2, 0xE3, 0xE4, 0xF7, 0xF8, 0xFA, 0xFD,
                  0xFE]:
            op_lengths[o] = 1
        for o in [0x35, 0x36, 0x37, 0x3D, 0x3E, 0x41, 0x42, 0x46, 0x50, 0x52, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x62,
                  0x63, 0x77, 0x78, 0x6C, 0x7D, 0x80, 0x81, 0x86, 0x87, 0x8D, 0x98, 0x9B, 0x9C, 0xA1, 0xA7, 0xB0, 0xB4,
                  0xB5, 0xB8, 0xB9, 0xBA, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, 0xDC,
                  0xDD, 0xE7, 0xF0, 0xF2, 0xF3, 0xF4, 0xF9]:
            op_lengths[o] = 2
        for o in [0x37, 0x3F, 0x40, 0x43, 0x44, 0x48, 0x4B, 0x4C, 0x4D, 0x5D, 0x5E, 0x5F, 0x60, 0x64, 0x65, 0x70, 0x71,
                  0x72, 0x7E, 0x7F, 0x84, 0x85, 0x8B, 0x8C, 0xBC, 0xEF, 0xF1]:
            op_lengths[o] = 3
